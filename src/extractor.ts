...
export interface ExtractOptions {
  url: string;
  selector?: string; // CSS selector for links/buttons
  downloadDir?: string;
  headless?: boolean;
  maxFiles?: number;
}
...
export async function extractAndDownload(options: ExtractOptions): Promise<ExtractedDownload[]> {
  ...
  const results: ExtractedDownload[] = [];
